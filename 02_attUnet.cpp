...
int main(){
    ...
    try {
        torch::jit::script::Module module = torch::jit::load(R"(C:\Users\ZY\CLionProjects\att_unet_cpp\attunet.pt)",at::kCUDA);
...
//        inputs.emplace_back(torch::ones({1, 3, 768, 768}));
        ...
        double t = (double)cv::getTickCount();
        {
            ...
            data = at::div(at::sub(data,0.5),0.5);
        }
...
//        at::Tensor out_tensor = module.forward( {torch::ones({1, 3, 768, 768}).to(at::kCUDA)} ).toTensor();
        ...
        cout<<"........2.........."<<endl;
        out_tensor = torch::sigmoid(out_tensor); //激活到0-1之间，网络模型中没有，所以在这里加
        cout<<"网络输出成功"<<endl;
